...
export class ShiftRequest {
    ...
    copyWeekSchedule = async (groupName: string, srcWeekStart: Dayjs, tgtWeekStart: Dayjs): Promise<CopyStatus> => {
        try {
            const response: AxiosResponse = await axios.post(process.env.EXPO_PUBLIC_API_URL + 'api/shift/preset',
                {
                    groupName: groupName,
                    srcWeekStart: srcWeekStart.format('YYYY-MM-DD'),
                    tgtWeekStart: tgtWeekStart.format('YYYY-MM-DD'),
                    mode: 'SKIP'
                }
            );
            return response.data;
